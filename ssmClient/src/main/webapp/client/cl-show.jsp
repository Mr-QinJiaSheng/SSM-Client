...
<div class="title" id="showNotice">
     <h2>数据字典管理</h2>
     <a onclick="xadmin.open('新建','<%=path%>/client/cl-add.jsp',700,500)" title="新建"><i class="add"></i></a>
</div>
  <div class="query">
    ...
    <%--   <input type="hidden" name="id" value="${data.id}"/> --%>
     <c:choose>   	    
         ...
         <tr class="mouse">
	        ...
	        <td ><a onclick="xadmin.open('编辑','<%=path %>/khclient/${data.id}/load',700,500)"  title="编辑" class="ico-edit"></a>
	             <a href="<%=path %>/khhui/${data.id}/xiang"  title="跟进记录"><i class="ico-schedule"></i></a>
	             <a href="<%=path %>/khclient/${data.id}/del" onclick="return del()" title="删除" class="ico-del"></a>
	        </td>    
	      </tr>
         </c:when>
