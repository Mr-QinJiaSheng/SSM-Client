...
<div class="title" >
    ...
    <thead>
      <tr>
        ...
        <th scope="col" class="wdh60">客户状态</th>
     ...
     <c:set var="nowDate" value="<%=System.currentTimeMillis()%>"></c:set>   
      <c:forEach var="data" items="${pager.data}">
        <form action="<%=path%>/khhui/update?oper=wei" name="crm"  method="post" >
	      ...
	      <tr class="mouse">
		   ...
		   <td >${data.khClientinfo.kuhuname}</td>
	        ...
	        <td>${data.yuday}</td>      
	          ...
	          <td ><input type="submit" class="ico-confirm" value=" " title="确定">
	            <a href="<%=path %>/khhui/${data.id}/del?oper=wei" onclick="return del()" title="删除" class="ico-del"></a>
	        </td>
	      </tr>
	      </form>
     </c:forEach>
