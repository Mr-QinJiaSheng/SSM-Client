...
<div class="title ue-clear">
    	<h2>人员管理&nbsp;--&gt;&nbsp;人员显示</h2>
    	<a onclick="xadmin.open('添加','<%=path%>/yhrole/doadd',380,350)" class="add" title="添加"></a>
    </div>
     ...
     <tbody>
      <c:forEach var="data" items="${pager.data}">
	      <tr class="mouse">
			...
			<td>${data.tel}</td>
	        <td>
	           <a onclick="xadmin.open('编辑','<%=path%>/yhrole/${data.id}/loads',380,300)" class="ico-edit" title="编辑"></a>
	           <a href="<%=path%>/yhrole/${data.id}/del" onclick="return del()" class="ico-del" title="删除"></a></td>
	      </tr>
     </c:forEach>
     </tbody>
    </table>
