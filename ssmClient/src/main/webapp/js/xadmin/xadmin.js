;!function (win) {
  ...
  Xadmin.prototype.end = function() {
	...
	Xadmin.prototype.add_tab = function (title,url,is_refresh) {
		...
		//重复点击
		for (var i = 1; i <$('.x-iframe').length; i++) {
  ...
  Xadmin.prototype.del_tab = function (id) {
	...
	Xadmin.prototype.open = function (title,url,w,h,full) {
	    ...
	    var index = layer.open({
	        ...
	        area: [w+'px', h +'px'],
	        fix: false, //不固定
	        maxmin:false,   //是否开启最大化
	        shadeClose: true,   //是否关闭遮盖  否
	        shade:0.6,    //遮盖层
	        title:false, //不显示标题栏    显示 写  title
	        moveOut: true,
	        content: url
...
layui.use(['layer','element','jquery'],function() {
    ...
    $('.left-nav #nav').on('click', 'li', function(event) {
        ...
        if($('.left-nav').css('width')=='50px'){
          $('.left-nav').animate({width: '170px'}, 100);
          $('.page-content').animate({left: '175px'}, 100);
    ...
    $('.container .left_open i').click(function(event) {
        if($('.left-nav').css('width')=='170px'){
            ...
            $('.left-nav').animate({width: '50px'}, 100);
            $('.left-nav cite,.left-nav .nav_right').hide();
            $('.page-content').animate({left: '55px'}, 100);
